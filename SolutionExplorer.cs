...
namespace XorNet.Rose 
{
    public partial class SolutionExplorer : System.Windows.Forms.UserControl 
	{
        ...
        public void AddAllNodes(bool showAll) 
		{
			treeView1.Nodes.Clear();
			treeView1.AppendNode(new object[] { "Rose Environment" }, -1, -1, -1, 3);
			treeView1.AppendNode(new object[] { "Windows" }, -1, -1, -1, 7);
			foreach (KeyValuePair<string, WindowProperty> pair in ResManager.Instance.Data.Properties)
			{
				treeView1.AppendNode(new object[] { pair.Key }, 1, -1, -1, 11);
			}
			/*
		...
		private void treeView1_KeyDown(object sender, KeyEventArgs e)
		{
			switch (e.KeyCode)
			{
				case System.Windows.Forms.Keys.Delete:
					{
						if(treeView1.Selection != null && treeView1.Selection.Count > 0)
						{
							foreach(TreeListNode node in treeView1.Selection)
							{
								if(node.ParentNode != null)
								{
									ResManager.Instance.Data.Properties.Remove(node.GetDisplayText(0));
									ResManager.Instance.Data.Apps.Remove(node.GetDisplayText(0));
								}
							}
							treeView1.DeleteSelectedNodes();
						}
					}
					break;
			}
